...
import type { MenuItemSummary, Ingredient, MenuItemRecipeEntry } from '../../types'
...
interface Props {
  ...
  onClose: () => void
  onSaved: (item: MenuItemSummary) => void
}

export default function EditMenuItemModal({ menuItem, onClose, onSaved }: Props) {
  const [itemName, setItemName] = useState(menuItem.menuItemName)
  const [originalRecipes, setOriginalRecipes] = useState<MenuItemRecipeEntry[]>([])
  ...
  useEffect(() => {
    Promise.all([
      api.getMenuItemRecipes(menuItem.menuItemId),
      api.getAllIngredients(),
    ])
      .then(([recipes, ings]) => {
        setOriginalRecipes(recipes)
        setPending(recipes.map(r => ({
          menuItemRecipeId: r.menuItemRecipeId,
          ingredient: {
            ingredientId: r.ingredientId,
            ingredientName: r.ingredientName,
            unitOfMeasure: r.unitOfMeasure,
          },
          adultPortion: r.adultPortion,
          youthPortion: r.youthPortion,
          kidPortion: r.kidPortion,
          codePortion: r.codePortion,
          notes: r.notes ?? '',
        })))
  ...
  function removeIngredient(ingredientId: number) {
    setPending(prev => prev.filter(p => p.ingredient.ingredientId !== ingredientId))
  }

  function updatePortion(ingredientId: number, field: 'adultPortion' | 'youthPortion' | 'kidPortion' | 'codePortion', value: number) {
    setPending(prev => prev.map(p => p.ingredient.ingredientId === ingredientId ? { ...p, [field]: value } : p))
  }

  function updateNotes(ingredientId: number, value: string) {
    setPending(prev => prev.map(p => p.ingredient.ingredientId === ingredientId ? { ...p, notes: value } : p))
  }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      if (itemName.trim() !== menuItem.menuItemName) {
        await api.updateMenuItem(menuItem.menuItemId, itemName.trim())
      }

      const currentRecipeIds = new Set(pending.filter(p => p.menuItemRecipeId).map(p => p.menuItemRecipeId!))

      // Serialize writes — SQLite only allows one writer at a time.
      for (const r of originalRecipes) {
        if (!currentRecipeIds.has(r.menuItemRecipeId)) {
          await api.deleteMenuItemRecipe(r.menuItemRecipeId)
        }
      }
      for (const p of pending) {
        if (p.menuItemRecipeId) {
          await api.updateMenuItemRecipe(p.menuItemRecipeId, p.adultPortion, p.youthPortion, p.kidPortion, p.codePortion, p.notes || null)
        } else {
          await api.addMenuItemRecipe(menuItem.menuItemId, p.ingredient.ingredientId, p.adultPortion, p.youthPortion, p.kidPortion, p.codePortion, p.notes || null)
        }
      }

      onSaved({ ...menuItem, menuItemName: itemName.trim() })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box modal-box-xwide">
        <h2 className="modal-title modal-title--menu-item"><Sandwich size={20} /> Edit Menu Item</h2>

        {loading ? (
          <p className="empty-state" style={{ padding: '2rem' }}>Loading...</p>
        ) : (
          <>
        ...
        <div className="modal-footer">
          ...
          <button type="button" className="btn-gold" disabled={loading || saving || !itemName.trim()} onClick={handleSave}>
